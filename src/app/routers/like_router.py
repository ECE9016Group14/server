...
async def create(line: LikeSchema, current_user: UserSchema = Security(get_current_active_user),service: LikeService = Depends()):
     line.id=None
...
async def update(line: LikeSchema,current_user: UserSchema = Security(get_current_active_user), service: LikeService = Depends()):
    return CommRes.success( service.update(line))

@like_router.delete("/delete_by_id/{id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_by_id(id: str, current_user: UserSchema = Security(get_current_active_user),service: LikeService = Depends()):
    return CommRes.success(service.delete(id))

@like_router.delete("/delete/{user_id}/{post_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete(user_id: str, post_id: str,current_user: UserSchema = Security(get_current_active_user), service: LikeService = Depends()):
